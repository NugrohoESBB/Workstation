    ...
    document.getElementById('panelForm').addEventListener('submit', function(e) {
      ...
      if (lokasi && kebutuhan && konfigurasi) {
        ...
        function hitungEstimasiHargaPanel(jenis, wpPanel) {
          const jumlah = hitungJumlahPanel(jenis, wpPanel);
          const harga = estimasiHargaPanelValues[jenis]?.[wpPanel];
          if (jumlah === 'X' || !harga) return 'X';
          return 'Rp. ' + (jumlah * harga).toLocaleString('id-ID');
        }
        ...
        // Tabel Jumlah dan Estimasi Biaya Baterai
        const bateraiRows = bateraiConfigs.map(cfg => {
          const jumlah = Math.ceil(kapasitasBaterai / (cfg.v * cfg.ah));
          const estimasi = jumlah * cfg.harga;
          return [cfg.v, cfg.ah, jumlah, 'Rp. ' + estimasi.toLocaleString('id-ID')];
        });
        buatTabel('tabelJumlahBaterai', 'Jumlah dan Estimasi Biaya Baterai', ['V', 'Ah', 'Jumlah', 'Estimasi Biaya'], bateraiRows);
        ...
        // Tabel Jumlah dan Estimasi Biaya Inverter
        const inverterRows = kapasitasPilihan.map(kapasitas => {
          const jumlah = Math.ceil(kapasitasInverter / kapasitas);
